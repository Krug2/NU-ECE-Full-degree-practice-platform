...
const variants=["invalid-argument","invalid-base","division-confusion","negative-output","boundary-inclusion","composition-domain","canceled-restriction","rounded-boundary","mixed"];
it.each(variants)("independently checks %s reasoning and preserves original restrictions",variant=>{
  ...
  expect([...flags].sort()).toEqual([0,1,2]);expect([...signs].sort()).toEqual([-1,1]);if(variant==="mixed")expect([...modes].sort()).toEqual([0,1,2,3,4,5,6,7]);
});
