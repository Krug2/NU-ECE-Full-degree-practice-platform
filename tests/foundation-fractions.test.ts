...
it("checks every simplification and operation against the original expression for fifty seeds", () => {
  for (let seed = 0; seed < 50; seed++) for (const [family, variants] of [["mth-rational-simplify", ["factors", "squares", "monomial"]], ["mth-rational-operations", ["add", "subtract", "shared", "repeated", "multiply", "divide", "complex"]]] as const) for (const variant of variants) {
    ...
    const excluded = variant === "factors" ? [String(a), String(c)] : variant === "squares" ? [String(n)] : variant === "monomial" ? ["0"] : variant === "repeated" ? [String(a)] : variant === "add" || variant === "subtract" || variant === "shared" ? [String(a), String(b)] : variant === "complex" ? [String(a), `(${a}*${k}-1)/${k}`] : [String(b), String(c)];
    ...
    for (let x = -10; x <= 10; x++) {
      ...
      const original = variant === "factors" ? `((${x}-${a})*(${x}-${b}))/((${x}-${a})*(${x}-${c}))` : variant === "squares" ? `((${x})^2-${n*n})/(${x}-${n})` : variant === "monomial" ? `(${g}*(${x})^${power})/(${h}*${x})` : variant === "repeated" ? `${u}/(${x}-${a})^2+${v}/(${x}-${a})` : variant === "shared" ? `${u}/(${x}-${a})+${v}/((${x}-${a})*(${x}-${b}))` : variant === "add" || variant === "subtract" ? `${u}/(${x}-${a})${variant === "subtract" ? "-" : "+"}${v}/(${x}-${b})` : variant === "complex" ? `(1/(${x}-${a}))/(1/(${x}-${a})+${k})` : variant === "divide" ? `((${x}-${a})/(${x}-${b}))/((${x}-${c})/(${x}-${b}))` : `((${x}-${a})/(${x}-${b}))*((${x}-${b})/(${x}-${c}))`;
