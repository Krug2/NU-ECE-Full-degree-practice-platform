...
const directory=new URL("../content/lessons/f09/",import.meta.url),files=readdirSync(directory).filter(f=>f.endsWith(".json"));
...
it.each(files)("%s contains full instruction, guided work and checked assessment coverage",file=>{
 ...
 expect(lesson.sections.length).toBeGreaterThanOrEqual(5);expect(lesson.examples.length).toBeGreaterThanOrEqual(7);math(lesson);
 ...
 for(const spec of[...lesson.practice,...lesson.checkpoint])for(let seed=0;seed<50;seed++){const q=f09Question(spec.familyId,spec.variant,"content-"+seed,"q");expect(q.objectiveId).toBe(lesson.id);expect(gradeQuestion(q,refresherAnswers(q)).correct).toBe(true);}
});
