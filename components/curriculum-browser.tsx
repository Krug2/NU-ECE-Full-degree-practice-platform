...
export function CurriculumBrowser() {
  ...
  return <>
    ...
    <div className="notice">We&apos;re building one learning path at a time. Available refresher previews contain lessons and practice; other entries remain curriculum outlines. Each learning path states its current scope and review status.</div>
    <div className="results-line"><span role="status">{results.length} {results.length === 1 ? "outline" : "outlines"} shown</span><span>32 NU courses · 12 optional refreshers</span></div>
    {results.length ? <div className="course-grid">{results.map(course => <CourseCard key={course.id} course={course} selected={data.plan.includes(course.id)} disabled={!ready || locked} onToggle={() => toggleCourse(course.id)} />)}</div> : <EmptyState icon="search" title="No matching outlines" action={<button className="button secondary" onClick={() => { setQuery(""); setSubject("all"); router.replace("/curriculum", { scroll: false }); }}>Clear filters</button>}>Try a course code or broaden your subject filter.</EmptyState>}
