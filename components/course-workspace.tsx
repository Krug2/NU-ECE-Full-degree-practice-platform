...
export function CourseWorkspace({ course, pack, lessonVersions,assessmentPanel,availabilityNote }: { course: Course; pack?: LearningPack; lessonVersions: Record<string,number>;assessmentPanel?:ReactNode;availabilityNote?:string }) {
  ...
  return <>
    ...
    <div className="two-columns section-space"><div className="stack">
      ...
      <section className="panel"><h2>{pack&&pack.status!=="building"?"Your study sequence":"A complete course, step by step"}</h2><ol className="step-list">{(pack&&pack.status!=="building"?[["Check preparation", "Use readiness results and prerequisite links to choose repairs."], ["Study and investigate", "Work through explanations, examples, guided tasks and investigations."], ["Assess and apply", "Demonstrate each objective, take course assessments and preserve project evidence."], ["Review over time", "Repair missed objectives and return for fresh retrieval practice."]]:[["Plan the scope", "Objectives, prerequisites, resource choices, and coverage."], ["Build the material", "Lessons, worked examples, practice, and relevant labs."], ["Review and verify", "Correctness, assessment quality, accessibility, and completeness."], ["Release the course", "A complete learning path you can work through at your pace."]]).map(([title,description],index) => <li key={title}><span className="step-number">{index+1}</span><div><strong>{title}</strong><p>{description}</p></div></li>)}</ol></section>
      ...
      <section className="panel"><div className="panel-heading"><h2>Explore references</h2><Icon name="external" size={17} /></div><p className="muted">{pack&&pack.status!=="building"?"Browse supporting references here. Each lesson links directly to the readings selected for its topic.":"Starting points for exploration. Exact sections will be selected during course planning."}</p>{resources.slice(0,3).map(resource => <a className="compact-link" key={resource.id} href={resource.url} target="_blank" rel="noreferrer"><div><strong>{resource.title}</strong><small>{resource.kind} · External resource</small></div><Icon name="external" size={14} /></a>)}<Link className="button quiet" href={`/resources?course=${course.id}`}>All related resources <Icon name="arrow" size={16} /></Link></section>
    </div></div>
