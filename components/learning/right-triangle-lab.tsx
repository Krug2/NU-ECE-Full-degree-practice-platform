...
export function RightTriangleLab({ activity }: { activity: { prompt: string; cases: RightTriangleCase[] } }) {
  ...
  return <div className="right-triangle-lab">
    ...
    {solved && result && <section className="right-results">
      ...
      <div className="field"><label htmlFor={id + "-reason"}>What changes when the reference angle switches?</label><select id={id + "-reason"} value={reason} onChange={event => { setReason(event.target.value); setExplanation(""); }}><option value="">Choose a relationship</option><option value="swap">Opposite and adjacent exchange, sine and cosine exchange, and BC remains the hypotenuse</option><option value="fixed">Opposite and adjacent stay attached to the same legs</option><option value="hyp">The hypotenuse becomes an adjacent leg</option></select></div>
      <div className="field"><label htmlFor={id + "-similarity"}>Why do the ratios stay the same after scaling?</label><select id={id + "-similarity"} value={similarity} onChange={event => { setSimilarity(event.target.value); setExplanation(""); }}><option value="">Choose a reason</option><option value="cancel">The same positive scale factor cancels from numerator and denominator</option><option value="equal">The side lengths do not change</option><option value="angle">The acute angles multiply by the scale factor</option></select></div>
