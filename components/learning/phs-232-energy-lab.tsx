...
const metric = (n: number) => n !== 0 && Math.abs(n) < .001 ? n.toExponential(5) : display(n);
const tick = (n: number) => n !== 0 && (Math.abs(n) < .01 || Math.abs(n) >= 10000) ? n.toExponential(1).replace(".0e", "e") : String(Number(n.toPrecision(3)));
const nullable = (n: number | null, unit = "") => n === null ? "not defined for this run" : `${metric(n)}${unit ? ` ${unit}` : ""}`;
const controls = [
  ...
  return <div className="phs232-investigation"><p><MathText>{activity.prompt}</MathText></p>
    ...
    {run && probe && <div className="notice">
      ...
      <div className="phs232-table" role="region" aria-label="Pendulum convergence table; scroll horizontally if needed" tabIndex={0}><table><caption>Same models and physical window, two step sizes</caption><thead><tr><th scope="col">Numerical measure</th><th scope="col">Coarse</th><th scope="col">Refined</th></tr></thead><tbody>
        ...
        <tr><th scope="row">Maximum nonlinear absolute energy drift (J)</th><td>{metric(run.coarse.energyDrift)}</td><td>{metric(run.fine.energyDrift)}</td></tr>
        ...
        <tr><th scope="row">Maximum linear absolute energy drift (J)</th><td>{metric(run.coarse.linearEnergyDrift)}</td><td>{metric(run.fine.linearEnergyDrift)}</td></tr>
      ...
      <p>Maximum nonlinear angle difference at aligned coarse times: {metric(run.maxAngleDifference)} rad. Refined minus coarse crossing period: {nullable(run.periodDifference, "s")}. Neither difference is a guaranteed error bound. Repeat with a finer step and check whether the quantity of interest settles.</p>
      ...
      <StateTable run={run}/>
      <p>Numbers are rounded for display. Tiny differences use scientific notation: 1e-6 means 0.000001 in the labeled unit. Floating-point and step errors can create small nonzero drift in this conservative model; they are not evidence of physical friction. Energy conservation alone also does not guarantee correct phase timing.</p>
