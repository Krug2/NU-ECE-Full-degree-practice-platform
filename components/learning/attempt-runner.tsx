...
function AttemptSession({attempt,onDraftCopied}:{attempt:Attempt;onDraftCopied:()=>void}) {
  ...
  const copyDraft=()=>{
    const copy:Attempt={...attempt,id:crypto.randomUUID(),mode:"practice",revision:0,status:"active",submittedAt:null,startedAt:new Date().toISOString(),responses,position};
    const saved=saveProgress(data=>({...data,learning:{...data.learning,attempts:[...data.learning.attempts,copy]}}));
    if(getStudySnapshot().data.learning.attempts.some(item=>item.id===copy.id))onDraftCopied();
...
export function AttemptRunner({lesson}:{lesson:Lesson}) {
  ...
  return <div><div className="tabs" role="group" aria-label="Learning mode"><button aria-pressed={mode==="practice"} onClick={()=>setMode("practice")}>Practice</button><button aria-pressed={mode==="checkpoint"} onClick={()=>setMode("checkpoint")}>Checkpoint</button></div><p className="muted">{mode==="practice"?`Work through ${current?.questions.length??lesson.practice.length} varied problems with hints and explanations. Generate a new set whenever you want more practice.`:"Four new questions check this objective independently. Demonstrate at least three correctly, including every required validity check. Feedback appears after submission."}</p>{!ready?<p>Loading saved practice...</p>:current?<AttemptSession key={current.id} attempt={current} onDraftCopied={()=>{setMode("practice");setMessage("Draft kept as practice. Start a new checkpoint for independent evidence.");}}/>:<p>No {mode} started for this lesson yet.</p>}{(!current||current.status!=="active")&&<button className="button" onClick={start} disabled={!ready||locked}>{current?"Start another":"Start"} {mode}</button>}<p className="form-status" role="status">{message}</p></div>;
}
