...
export function AttemptRunner({lesson,onlyMode,showHistory=false}:{lesson:AssessmentSource;onlyMode?:Attempt["mode"];showHistory?:boolean}){
  ...
  const checkpointCopy=lesson.assessment?"This form has "+lesson.checkpoint.length+" questions. Meet each objective's published target and every critical check. Feedback appears after submission. Readiness reports preparation only.":lesson.checkpoint.length===4?"Four new questions check this objective independently. Demonstrate at least three correctly, including every required validity check. Feedback appears after submission.":"This checkpoint has "+lesson.checkpoint.length+" new questions. Demonstrate at least "+Math.ceil(.75*lesson.checkpoint.length)+" correctly, including every required validity check. Feedback appears after submission.";
  return <div>
