...
export function PracticalLessonReader({lesson,previous,next}:{lesson:PracticalLesson;previous?:{id:string;title:string};next?:{id:string;title:string}}){
 ...
 <Guided lesson={lesson}/><PracticalTask lesson={lesson}/>
 ...
 </div><aside className="lesson-nav"><span className="eyebrow">In this lesson</span><nav aria-label="Lesson sections">{[["read","Read and understand"],["examples","Worked examples"],["guided","Guided reasoning"],["practical-task","Practical task and saved work"],["review","Summary and recall"],["notes","Your notes"]].map(([id,label])=><a key={id} href={"#"+id}>{label}</a>)}</nav></aside></div></>;
}
