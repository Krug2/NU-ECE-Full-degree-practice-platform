...
export function inverseQuestion(familyId:string,variant:string,seed:string,id:string):Question{
  ...
  if(familyId==="mth-inverse-rule"){
    ...
    const fields=variant==="quadratic"?[
      {id:"formula",kind:"choice",label:"Inverse rule",correct:"inverse",options:rng.shuffle([{id:"inverse",label:"$f^{-1}(x)="+correct+"$",accessibleLabel:"f inverse of x equals "+h+" plus "+side+" times the square root of ((x minus "+k+") divided by "+sign+")",feedback:"This root sign returns inputs to the stated original branch."},{id:"branch",label:"$f^{-1}(x)="+other+"$",accessibleLabel:"f inverse of x equals "+h+" plus "+(-side)+" times the square root of ((x minus "+k+") divided by "+sign+")",feedback:"This root sign returns inputs on the opposite branch."},{id:"shift",label:"$f^{-1}(x)="+shift+"$",accessibleLabel:"f inverse of x equals "+(-h)+" plus "+side+" times the square root of ((x minus "+k+") divided by "+sign+")",feedback:"Undo the square around h, then add h back."},{id:"reciprocal",label:"$f^{-1}(x)=\\frac{1}{"+original+"}$",accessibleLabel:"f inverse of x equals one divided by ("+sign+" times (x minus "+h+") squared plus "+k+")",feedback:"The reciprocal of an output is not the inverse function."}])},
      ...
      {id:"root-step",kind:"choice",label:"Root step in the reverse composition",correct:side>0?"right":"left",options:rng.shuffle([{id:"right",label:"$\\sqrt{(x-("+h+"))^2}=x-("+h+")$ on this domain",accessibleLabel:"The square root of ((x minus "+h+") squared) equals x minus "+h+" on this domain",feedback:"This holds on the right branch, where x - h is nonnegative."},{id:"left",label:"$\\sqrt{(x-("+h+"))^2}=("+h+")-x$ on this domain",accessibleLabel:"The square root of ((x minus "+h+") squared) equals "+h+" minus x on this domain",feedback:"This holds on the left branch, where x - h is nonpositive."},{id:"both",label:"Either root sign may be chosen separately for each input",feedback:"The stated branch fixes a single inverse function; the principal square root is nonnegative."}])}
    ]:[
