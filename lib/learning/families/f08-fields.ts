...
export const cp=(id:string,label:string,expected:string)=>({id,kind:"polynomial" as const,label,expected:formatPolynomial(parsePolynomial(expected)),form:"equivalent" as const,unit:"",help:"Use x as the variable. Equivalent expanded or factored polynomial forms are accepted."});
