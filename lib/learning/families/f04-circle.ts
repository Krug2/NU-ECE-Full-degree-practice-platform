...
export function f04CircleQuestion(familyId: string, variant: string, seed: string, id: string): Question {
  ...
  if (familyId === "f04-circular-measure") {
    ...
    return questionSchema.parse({ ...base, category:"application", parameters:{radius,angleDegrees,turns,time},
      ...
      hints:["Use radian measure in s=r*theta and sector area=(1/2)*r²*theta.","Each full turn is 2*pi radians. Angular speed is angle/time; tangential speed is radius times angular speed.",`Here the angle coefficient is ${coefficient}; the angular-speed coefficient is ${omega}. Multiply these by radius where the formula requires it.`],
