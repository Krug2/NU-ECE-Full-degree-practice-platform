...
export function phs231CollisionQuestion(familyId:string,variant:string,seed:string,id:string){
  ...
  return finish({mA,mB,a,b,kind,A,B,den},`A is left of B at an impenetrable 1D contact. Masses are ${mA}, ${mB} kg; incoming velocities are ${a}, ${b} m/s. A record claims zero external impulse and lists outgoing velocities ${A}/${den} and ${B}/${den} m/s. All material is included. Calculate ΔP and ΔK, then audit the record's physical interpretation. Internal energy release is possible only if an appropriate store is supplied.`,
    [field("momentum","Total momentum change",`${dp}/${den}`,"kg m/s"),field("kinetic","Total kinetic-energy change",`${change}/${2*den*den}`,"J"),choice("audit","Interpretation of the proposed outgoing state",correct,[
      ...
      {id:"release",label:"Momentum-consistent separation that needs an internal energy release",feedback:"If ΔP=0 and ΔK>0, the increase must be supplied by an internal store or another accounted transfer; it is not a passive loss-only collision."},
