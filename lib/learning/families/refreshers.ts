import { f10FamilyIds, f10Question } from "./f10";
import { f09FamilyIds, f09Question } from "./f09";
...
export const refresherFamilyIds = [...f01FamilyIds, ...f02FamilyIds, ...f03FamilyIds, ...f04FamilyIds, ...f05FamilyIds, ...f06FamilyIds, ...f07FamilyIds, ...f08FamilyIds, ...f09FamilyIds, ...f10FamilyIds];
export function refresherQuestion(familyId: string, variant: string, seed: string, id: string) {
  if (familyId.startsWith("f10-")) return f10Question(familyId, variant, seed, id);
  if (familyId.startsWith("f09-")) return f09Question(familyId, variant, seed, id);
