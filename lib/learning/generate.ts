...
export const availableFamilyIds = new Set([...linearFamilyIds,...inequalityFamilyIds,...quadraticFamilyIds]);
export function generateQuestions(slots: QuestionSlot[], seed: string): Question[] {
  ...
  return slots.map((slot, index) => {
    ...
    for (let retry = 0; retry < 40; retry++) {
      const generate=linearFamilyIds.includes(slot.familyId)?linearQuestion:inequalityFamilyIds.includes(slot.familyId)?inequalityQuestion:quadraticQuestion;
