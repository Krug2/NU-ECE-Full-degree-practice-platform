...
export default function PlanPage() {
  ...
  const logSession = (event: FormEvent<HTMLFormElement>) => {
    ...
    const saved = saveProgress(current => ({ ...current, sessions: [...current.sessions, session] }));
  ...
  return <>
    ...
    <div className="two-columns"><div className="stack">
      ...
      <section className="panel"><h2>Record a study session</h2><p className="muted">A small step still counts. Log the time you spent studying today.</p>{data.plan.length ? <form onSubmit={logSession}><div className="field"><label htmlFor="session-course">Course or refresher</label><select id="session-course" name="course" required disabled={!ready || locked}>{data.plan.map(id => <option key={id} value={id}>{courseById(id)!.code} · {courseById(id)!.title}</option>)}</select></div><div className="field"><label htmlFor="session-minutes">Minutes studied</label><input id="session-minutes" name="minutes" type="number" min="1" max="480" step="1" defaultValue="30" required disabled={locked} /></div><div className="field"><label htmlFor="session-note">What did you work on? <span className="muted">(optional)</span></label><input id="session-note" name="sessionNote" maxLength={300} placeholder="Reviewed derivatives and worked examples" disabled={locked} /></div><button className="button" disabled={locked || !ready}>Save session <Icon name="check" size={17} /></button><p role="status" className="form-status">{message}</p></form> : <p className="notice">Add a course to your plan to record a session.</p>}</section>
    </div></div>
  </>;
}
